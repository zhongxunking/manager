...
public class SignFilter implements Filter {
    ...
    private boolean validateSign(String managerId, String sign, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (manager == null) {
            response.getWriter().append(format(Status.FAIL, CommonResultCode.INVALID_PARAMETER.getCode(), String.format("管理员[%s]不存在", managerId)));
            return false;
        }
        if (manager.getSecretKey() == null) {
            response.getWriter().append(format(Status.FAIL, CommonResultCode.INVALID_PARAMETER.getCode(), String.format("管理员[%s]未设置密钥", managerId)));
            return false;
        }
        ...
        if (!Objects.equals(sign, correctSign)) {
            response.getWriter().append(format(Status.FAIL, CommonResultCode.INVALID_PARAMETER.getCode(), "签名不正确"));
            return false;
        }
    ...
    // 格式化返回结果
    private String format(Status status, String code, String message) {
        return String.format("{\"status\":\"%s\",\"code\":\"%s\",\"message\":\"%s\",\"success\":%b}",
                status,
                code,
                message,
